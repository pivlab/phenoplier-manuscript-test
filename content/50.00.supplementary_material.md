...
Step 1 - GWAS.
We performed standard QC procedures such as filtering out variants with missing call rates exceeding 0.01, MAF below 1% or MAC below 20, and HWE below 1e-6, and removing samples with high sex-discrepancy and high-relatedness (first and second degree).
...
These steps include harmonization of genetic studies and imputation of $z$-scores, which are part of the TWAS pipeline and are needed to ensure an acceptable overlap with SNPs in prediction models.
...
We also detected other latent variables (LVs) with higher-than-expected mean type I errors for both the generalized least squares (GLS) and ordinary least squares (OLS) models, although they do not have a relatively large number of adjacent genes at the top.
One example is LV914, as shown in Figure @fig:reg:nulls:qqplot:lv914.
...
We did not see signs of inflation when applying the method in real data (Figure @fig:reg:real:qqplots).
...
For the first scenario, we shuffled genes in $\mathbf{M}$ for each trait, and then projected this randomized matrix into the latent space.
In the second scenario, we projected matrix $\mathbf{M}$ into the latent space, and then shuffled LVs in $\hat{\mathbf{M}}$ for each trait.
In each of these scenarios, we ran the same clustering pipeline used for the real data ([Methods](#sec:methods:clustering)), generating an ensemble of partitions that were later combined using consensus functions to derive the final partitions of traits.
